...
class ListaMetas extends React.Component {    
  ...
  onAprobarClick = () => {
    for(let i=0; i < this.state.idMetasA.length ; i++){ 
        axios.put(`http://146.83.216.224:5000/metas/setaprobado/${this.state.idMetasA[i]}`)
    }
    for(let i=0; i < this.state.idMetasD.length ; i++){ 
        axios.delete(`http://146.83.216.224:5000/metas/deletemetas/${this.state.idMetasD[i]}`)
    }
  }

  onRechazarClick = () => {
    for(let i=0; i < this.state.idMetasA.length ; i++){ 
        axios.delete(`http://146.83.216.224:5000/metas/deletemetas/${this.state.idMetasA[i]}`)
    }
    for(let i=0; i < this.state.idMetasD.length ; i++){ 
        axios.put(`http://146.83.216.224:5000/metas/setaprobado/${this.state.idMetasD[i]}`)
    }
  }
